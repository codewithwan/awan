...
export function StepStory({ story, beat, cast, solo, onStory, onBeat, onCast, onSolo }: Props) {
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[1.4fr_1fr]">
      <div className="flex flex-col gap-4">
        <Reel story={shown} toml={castOf(cast).toml} onBeat={(i) => onBeat(solo >= 0 ? solo : i)} />
        <Meter story={story} at={beat} onPick={onSolo} />
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        <Card title="Running order" hint="drag to reorder · ▶ plays one alone" tone="text-lime">
          <SceneList story={story} playing={beat} solo={solo} onChange={onStory} onSolo={onSolo} />
        </Card>
